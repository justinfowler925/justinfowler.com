...
## Claude Cowork distribution

Use this method for the task and working folder the user has actually provided. Fowler Brain and Brutus/Canon are optional existing integrations; this upload does not install them or grant access to Justin's systems. Follow the original unavailable-source fallback when they are absent: keep the small execution contract in the task, continue safe authorized work, and never claim external state was synchronized. Do not install private infrastructure or ask the rep to configure it just to use this skill.

The bundled scripts/intent_contract.py and references/contract-schema.md support optional structured validation when Python and file access are available. Ordinary tasks can keep the contract as working notes, as the method permits. Use the connected sources and tools actually available in Cowork; qualify claims about changes, saved files and verification by the evidence you can inspect.
